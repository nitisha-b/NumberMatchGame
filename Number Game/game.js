...
function draw() {  
  ...
  if(frameCount === 1 | frameCount % interval === 0){
    solution = null;
    guessItem = new GuessItem(width/2, height/2, 10); //object
  }
...
function solutionMessage(seed, solution){
  ...
  text(messages[parseInt(random(messages.length))], 0, 0);
  randomSeed();
  pop();
}
...
function displayGameOver(score){
  ...
  var alternaingValue = map(sin(frameCount/10), -1, 1, 0, 255);
  fill(0, 0, 255, alternaingValue);
  textSize(16);
  translate(0, 100);
